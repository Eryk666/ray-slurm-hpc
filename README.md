...
Download and unzip the [Polish Companies Bankruptcy](https://archive.ics.uci.edu/dataset/365/polish+companies+bankruptcy+data).

```sh
wget https://archive.ics.uci.edu/static/public/365/polish+companies+bankruptcy+data.zip -O $SCRATCH/data.zip
unzip $SCRATCH/data.zip -d $SCRATCH/
```

Adjust the `DATA_PATH` variable in both `baselline_job.sh` and `ray_job.sh` files to point to the extracted `1year.arff` file.

Use the `run_experiments.sh` script to execute the benchmark for 2, 4, 6, 8, 10, and 12 nodes.

Generate a scaling plot (Total Wall-clock Time vs. Number of Nodes) comparing both methods. Include this plot in your report along with answers to the following questions:
